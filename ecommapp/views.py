...
from ecommapp.models import Product, Cart,OrderA
    ...
    if request.method=='GET':
        ...
        if n=='' or p=='' or cp=='':
            ...
            try:
                ...
                context['errmsg']='User Alreday Exists, Please Login..!'
                return render(request,'register.html',context)
...
def placeorder(request):
    ...
    for x in c:
        amount=x.qty*x.pid.print
        o=OrderA.objects.create(orderid=orderid,pid=x.pid,uid=x.uid,qty=x.qty, amt=amount)
        o.save()
...
def fetchorder(request):
    o=OrderA.objects.filter(uid=request.user.id)
    context={'data':o}
    sum=0
    for x in o:
        sum=sum+x.amt
...
def makepayment(request):
    client = razorpay.Client(auth=("rzp_test_qEorKRj2pzEpHk", "SnHwF0ZwAoVYHheOBMlfKtrn"))
...
def paymentsuccess(request):
    sub='GPcart Order Status'
    msg='Thanks for Shopping..!!'
    frm='paitwargitesh74@gmail.com' 
    u=User.objects.filter(id=request.user.id)
    to=u[0].email
    send_mail(
        sub,
